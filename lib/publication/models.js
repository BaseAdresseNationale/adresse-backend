...
async function createSubmission({url, file}) {
  ...
  const {parseOk, rows, isValid} = await validate(data)

  if (!parseOk) {
    throw new Error('Fichier CSV contenant des erreurs')
  }
  ...
  const communes = uniq(rows.map(r => r.additionalValues.cle_interop.codeCommune))
