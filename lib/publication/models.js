...
const createPublicationNotificationEmail = require('../emails/publication-notification')
...
async function publishSubmission(submissiondId) {
  const submission = await updateSubmission(submissiondId, {status: 'published'})

  const {commune} = submission
  if (commune.email) {
    const email = createPublicationNotificationEmail({commune})
    await sendMail(email, [submission.commune.email])
  }
...
module.exports = {getSubmission, updateSubmission, createSubmission, publishSubmission, getPublishedSubmissions}
