...
app.route('/submissions/upload')
  .post(text({limit: '10mb', type: 'text/csv'}), w(async (req, res) => {
    if (!req.body) {
      return res.status(400).send({code: 400, message: '`file` est un champ obligatoire.'})
    }

    if (typeof req.body === String) {
      return res.status(400).send({code: 400, message: 'Le fichier n’a pas pu être analysé.'})
    }
