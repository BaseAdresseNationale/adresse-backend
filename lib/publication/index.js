...
const {getSubmission, updateSubmission, createSubmission, publishSubmission, getPublishedSubmissions} = require('./models')
...
app.route('/submissions/:submissionId/publish')
  .post(w(async (req, res) => {
    ...
    const submission = await publishSubmission(req.submission._id)
    res.send(submission)
  }))
