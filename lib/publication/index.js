...
app.route('/submissions/upload')
  .post(raw({limit: '10mb', type: 'text/csv'}), w(async (req, res) => {
    if (!Buffer.isBuffer(req.body)) {
      return res.status(400).send({code: 400, message: 'La fichier soumis n’a pas pu être traité. Vous devez soumettre un fichier CSV de moins de 10 Mo.'})
    }
...
app.route('/submissions/:submissionId/data')
  .get((req, res) => {
    ...
    res.type('text/csv').send(req.submission.data.toString())
  })
