...
const {publishSubmission, getSubmission, updateSubmission, createSubmission, getPublishedSubmissions, getPublishedSubmissionsUrls} = require('./models')
...
app.route('/submissions/published-urls')
  .get(w(async (req, res) => {
    const urls = await getPublishedSubmissionsUrls()
    res.send(urls)
  }))
