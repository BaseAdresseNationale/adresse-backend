...
<body>
  ...
  <div class="title">
    <h2 style="margin:0; mso-line-height-rule:exactly;">Publication de la Base Adresse Locale de <%= commune.nom %>
    </h2><br>
  ...
  <p>
    Une personne souhaite s’authentifier en tant qu’agent ou élu habilité afin de publier la Base Adresse Locale de la commune de <%= commune.nom %>.
      Si vous n'êtes pas à l'origine de cette demande, merci d'ignore ce courriel.
  </p>
...
function formatEmail(data) {
  const {code, commune} = data
  const apiUrl = getApiUrl()

  return {
    subject: 'Demande de code d’identification',
    html: bodyTemplate({code, commune, apiUrl})
  }
}
