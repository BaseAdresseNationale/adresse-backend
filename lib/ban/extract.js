...
const {codeCommuneIsValid, getCodeDepartement} = require('../cog')
...
function convert(adresseBan, proj) {
  ...
  const adresseBal = {
    ...
    long: '',
    lat: '',
    source: 'Base Adresse Nationale v1',
...
async function extractFromDepartement(codeDepartement, codesCommunes, outputStream) {
  ...
  const adresses = await getStream.array(
    pumpify.obj(
      ...
      through2.obj((adresseBan, enc, cb) => {
        if (!codesCommunes.includes(adresseBan.code_insee)) {
          return cb()
        }
...
async function createExtraction(codesCommunes) {
  ...
  }).then(() => csvStream.end()).catch(err => csvStream.destroy(err))
  return csvStream
}
...
module.exports = {convert, createExtraction, roundPrecision}
